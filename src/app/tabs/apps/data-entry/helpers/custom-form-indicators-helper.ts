...
export function evaluateCustomFomAggregateIndicators(indicators: any[]) {
  indicators.map((indicator: any) => {
    ...
    try {
      value = new Function(`return ( ${generatedExpresion})`)();
      if (isNaN(value)) {
        value = '-';
      } else {
...
function getDataElementTotalValue(dataElementId: string) {
...
function createAndExecuteIfStatement(elem: any, elementValues: any) {
  const trueValue = elem.split(',')[1];
  const falseValue = elem.split(',')[2].split(')')[0];
...
function getFalseValue(falseValue: any, elementValues: any) {
...
function getRightSideValue(elem: any, logicalOperator: any) {
  // remove spaces
  const newElem = elem
    .replace(/\t/g, '')
    .replace(/\n/g, '')
...
function splitExp(expression: string) {
  return expression
    .replace(/\+/g, 'SPC-')
    .replace(/\//g, 'SPC-')
    .replace(/\*/g, 'SPC-');
...
function getEvaluatedIndicatorValueFromExpression(
  ...
  if (expression && expression.indexOf('d2:condition') === -1) {
    ...
    _.map(splitExp(expression).split('SPC-'), (elem: any) => {
      // create if statement
  ...
  try {
    if (!isNaN(new Function(`return ( ${expression})`)())) {
      evaluatedValue = new Function(`return ( ${expression})`)();
    }
  } catch (e) {}
