...
function getDataValue(data: any, id: string) {
  ...
  if (dataSetReportAggregateValues) {
    ...
    for (const elementIdObject of elementIdObjects) {
      try {
        ...
        const optionCodeNameElement = document.createElement('span');
    ...
    // set previousDisabledFields on global letiables
    dhis2['previousDisabledFields'] = previousDisabledFields;

    // update option sets
    for (const elementId of Object.keys(elementsWithOptionSet)) {
      try {
        const inputElement: any = document.getElementById(elementId);
    ...
    // update option sets
    for (const elementId of Object.keys(elementsWithRadioInput)) {
      try {
        const inputElement: any = document.getElementById(elementId);
    ...
    // update text area
    for (const elementId of Object.keys(elementsWithTextArea)) {
      try {
        const inputElement: any = document.getElementById(elementId);
...
export function lockingEntryFormFields(shouldLockFields: boolean) {
