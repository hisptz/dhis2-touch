    ...
    return Observable.create(observer => {
      if (this._userInfo !== null) {
        ...
        const userInfoCall: Observable<any> = priority === false ?
          this.http.get('me.json?fields=dataViewOrganisationUnits[id,name,level],organisationUnits[id,name,level]',true) :
          this.http.get('me.json?fields=organisationUnits[id,name,level]',true);
  ...
  getOrgunitLevelsInformation() {
    return Observable.create(observer => {
      if (this.orgUnitLevels.length !== 0) {
        ...
        this.http.get('organisationUnitLevels.json?fields=id,name,level&order=level:asc',true)
  ...
  getOrgunitGroups() {
    return Observable.create(observer => {
      if (this.orgUnitGroups.length !== 0) {
        ...
        this.http.get('organisationUnitGroups.json?fields=id,name&paging=false',true)
  ...
  getAllOrgunitsForTree1(fields = null, orgunits = null) {
    return Observable.create(observer => {
      if (this.nodes !== null) {
        ...
        this.http.get('organisationUnits.json?fields=' + fields + '&filter=id:in:[' + orgunits.join(',') + ']&paging=false',true)
          .subscribe((nodes: any) => {
    ...
    return Observable.create(observer => {
      if (this.initialOrgUnits !== null) {
        ...
        this.http.get('organisationUnits.json?fields=id,name,level,children[id,name]&' +
          'filter=id:in:[' + orgunits.join(',') + ']&paging=false',true)
          .subscribe((nodes: any) => {
