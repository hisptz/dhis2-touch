...
export class DataFilterService {
  ...
  getIndicators(): Observable<Indicator[]> {
    return this.http.get('indicators.json?fields=id,name,dataSets[periodType]&paging=false',true)
  ...
  getDataElements(): Observable<DataElement[]> {
    return this.http.get('dataElements.json?fields=,id,name,valueType,categoryCombo&paging=false&filter=' +
      'domainType:eq:AGGREGATE&filter=valueType:ne:TEXT&filter=valueType:ne:LONG_TEXT',true)
      .map(res => res.dataElements || []);
  }

  getDataSets(): Observable<DataSet[]> {
    return this.http.get('dataSets.json?paging=false&fields=id,name',true)
      .map(res => res.dataSets || []);
  }

  getCategoryCombos(): Observable<CategoryCombo[]> {
    return this.http.get('categoryCombos.json?fields=id,name,categoryOptionCombos[id,name]&paging=false',true)
      .map(res => res.categoryCombos || []);

  }

  getOrganisationUnits(): Observable<any[]> {
    return this.http.get('organisationUnits.json?fields=id,name,children,parent,path&paging=false',true)
      .map(res => res.organisationUnits || []);
  }

  getDataElementGroups(): Observable<DataelementGroup[]> {
    return this.http.get('dataElementGroups.json?paging=false&fields=id,name,dataElements[id,name,categoryCombo]',true)
      .map(res => res.dataElementGroups || []);
  }

  getIndicatorGroups(): Observable<IndicatorGroup[]> {
    return this.http.get('indicatorGroups.json?paging=false&fields=id,name,indicators[id,name]',true)
      .map(res => res.indicatorGroups || []);
  }

  getPrograms(): Observable<any[]> {
    return this.http.get('programs.json?paging=false&fields=id,name,programType,programIndicators[id,name',true)
      .map(res => res.programs || []);

  }

  getProgramIndicators(): Observable<any[]> {
    return this.http.get('programIndicators.json?paging=false&fields=id,name',true)
